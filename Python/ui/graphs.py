...
def build(state, parent, fonts):
    ...
    line_temp, = ax_t.plot([], [], color=ACCENT,  lw=2,   label="Temperature")
    line_setp, = ax_t.plot([], [], color=ACCENT2, lw=1.5, linestyle="--", label="Setpoint")
    ax_t.legend(fontsize=8, loc="upper right", facecolor=PANEL, edgecolor=BORDER)
    ...
    line_volt, = ax_vi.plot([], [], color=PURPLE, lw=2, label="Voltage")
    ax_vi2 = ax_vi.twinx()
    ...
    line_curr, = ax_vi2.plot([], [], color=YLWDK, lw=2, label="Current")
    ax_vi.legend([line_volt, line_curr], ["Voltage (V)", "Current (A)"],
                 fontsize=8, loc="upper right", facecolor=PANEL, edgecolor=BORDER)
    ax_vi.set_title("Voltage & Current vs Time", fontsize=9, color=TEXT_DIM, pad=3)
    ...
    line_p, = ax_pid.plot([], [], color=ACCENT,  lw=1.5, label="P")
    line_i, = ax_pid.plot([], [], color=TEAL,    lw=1.5, label="I")
    line_d, = ax_pid.plot([], [], color=ORANGE,  lw=1.5, label="D")
    ax_pid.legend(fontsize=8, loc="upper right", facecolor=PANEL, edgecolor=BORDER)
    ax_pid.set_title("PID Components vs Time", fontsize=9, color=TEXT_DIM, pad=3)

    state.fig=fig; state.ax_t=ax_t; state.ax_vi=ax_vi
    state.ax_vi2=ax_vi2; state.ax_pid=ax_pid
    state.line_temp=line_temp; state.line_setp=line_setp
    state.line_volt=line_volt; state.line_curr=line_curr
    state.line_p=line_p; state.line_i=line_i; state.line_d=line_d
    ...
    state.btn_live = tk.Button(toolbar_frame, text="● LIVE",
    ...
    def _reset_view():
        with state.glock:
            if not state.g_times: return
            t0,t1=state.g_times[0],state.g_times[-1]
            tp_all=list(state.g_temps); sp_all=list(state.g_setpts)
        pad_x=max((t1-t0)*0.02,5); pad_y=2
    ...
    tk.Button(toolbar_frame, text="⤢  FULL VIEW",
        ...
        padx=10, pady=3, command=_reset_view).pack(side="right", padx=2, pady=3)
    ...
    def _on_scroll(event):
        ...
        xdata=event.xdata or 0; ydata=event.ydata or 0
        for a in [ax_t,ax_vi,ax_vi2,ax_pid]:
            xl=a.get_xlim()
            a.set_xlim(xdata+(xl[0]-xdata)*factor, xdata+(xl[1]-xdata)*factor)
    ...
    def _upd(frame):
        with state.glock:
            if len(state.g_times)<2: return
            t=list(state.g_times);  tp=list(state.g_temps)
            sp=list(state.g_setpts)
            vo=list(state.g_volts); cu=list(state.g_currs)
            pp=list(state.g_pid_p); pi=list(state.g_pid_i); pd=list(state.g_pid_d)

        line_temp.set_data(t,tp); line_setp.set_data(t,sp)

        if t:
            if state.graph_live[0]:
                w=state.graph_window[0]
                x_min=max(0,t[-1]-w); x_max=t[-1]+5
                ax_t.set_xlim(x_min,x_max)
                ax_t.set_ylim(min(min(tp),min(sp))-2, max(max(tp),max(sp))+2)
            else:
            ...
            if state.hold_regions:
                ...
                for region in state.hold_regions:
                    ...
                    if h_end_draw<=x_min or h_start>=x_max: continue
                    span=Rectangle((h_start,y_lo),h_end_draw-h_start,y_hi-y_lo,
                        linewidth=0,facecolor="#1e8449",alpha=0.13,
                        zorder=0,transform=ax_t.transData,clip_on=True)
                    ax_t.add_patch(span)
                    vl_s,=ax_t.plot([h_start,h_start],[y_lo,y_hi],
                        color="#1e8449",linestyle="--",linewidth=1.2,
                        zorder=2,transform=ax_t.transData)
                    vl_e=None
                    if h_end is not None:
                        vl_e,=ax_t.plot([h_end,h_end],[y_lo,y_hi],
                            color="#c0392b",linestyle="--",linewidth=1.2,
                            zorder=2,transform=ax_t.transData)
                    txt=None
                    lx=max(h_start+0.5,x_min+0.5)
                    if lx<x_max and lx<h_end_draw:
                        st2="" if h_end is None else " ✓"
                        txt=ax_t.text(lx,y_hi-(y_hi-y_lo)*0.05,
                            f"Hold {step_num}{st2}",fontsize=7.5,color="#1e8449",
                            va="top",ha="left",zorder=3,transform=ax_t.transData,
                            bbox=dict(boxstyle="round,pad=0.2",facecolor="#eafaf1",
                                edgecolor="#1e8449",alpha=0.80,linewidth=0.7))
                    state._hold_artists.append((span,vl_s,vl_e,txt))

        line_volt.set_data(t,vo); line_curr.set_data(t,cu)
        if t and state.graph_live[0]:
            w=state.graph_window[0]
            ax_vi.set_xlim(max(0,t[-1]-w), t[-1]+5)
            ax_vi2.set_xlim(max(0,t[-1]-w), t[-1]+5)
            v_lo=min(vo); v_hi=max(vo); vr=max(v_hi-v_lo,0.5)
            ax_vi.set_ylim(v_lo-vr*0.2, v_hi+vr*0.2)
            i_lo=min(cu); i_hi=max(cu); ir=max(i_hi-i_lo,0.05)
            ax_vi2.set_ylim(i_lo-ir*0.2, i_hi+ir*0.2)

        line_p.set_data(t,pp); line_i.set_data(t,pi); line_d.set_data(t,pd)
        if t and pp and state.graph_live[0]:
            ax_pid.set_xlim(max(0,t[-1]-state.graph_window[0]), t[-1]+5)
            all_pid=pp+pi+pd
            p_lo=min(all_pid); p_hi=max(all_pid); pr=max(p_hi-p_lo,0.1)
            ax_pid.set_ylim(p_lo-pr*0.2, p_hi+pr*0.2)

        mpl_canvas.draw_idle()

    state.ani=FuncAnimation(fig,_upd,interval=1000,blit=False,cache_frame_data=False)